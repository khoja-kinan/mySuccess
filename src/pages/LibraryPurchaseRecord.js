...
import {
  ...
  LibrarySearchPurchaseRecordByDate,
  LibraryPurchaseRecordURL,
} from "../constants/urls";
...
export default function LibraryPurchaseRecord() {
  ...
  function fecthData() {
    if (token === null) {
      ...
      axios
        .get(`${LibraryPurchaseRecordURL}${user_id}`, {
          headers: {
  ...
  function applySortFilter(array, comparator, query) {
    ...
    if (query) {
      return filter(
        array,
        (_user) =>
          _user.charger.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
  ...
  const handleSearch = () => {
    ...
    axios
      .get(
        `${LibrarySearchPurchaseRecordByDate}${user_id}/${FromDate}/${ToDate}`,
        {
          headers: {
            Authorization: "Bearer " + token,
            Accept: "application/json",
  ...
  return (
    <Page title="لوحة التحكم | سجل شراء المكتبة">
      <Container>
        <Stack
          direction="row"
          alignItems="center"
