...
export default function SupportViderMoreMenu({
  user,
  token,
  setLoadingBtn,
  loadingBtn,
  ...
  */
  ...
  const [VideoName, setVideoName] = useState(user.tittle);
  const [VideoTeacher, setVideoTeacher] = useState(user.teacher_name);
  ...
  const handleSubmitEditUnit = () => {
    ...
    VideoName !== user.tittle && formData.append("tittle", VideoName);
    VideoBio !== user.bio && formData.append("bio", VideoBio);
    VideoTeacher !== user.teacher_name &&
      formData.append("teacher_name", VideoTeacher);
    ...
    axios
      .post(`${supportURL}/${user.id}?_method=PUT`, formData, {
        headers,
      })
      .then((response) => {
        setOpenEditUnit(false);
