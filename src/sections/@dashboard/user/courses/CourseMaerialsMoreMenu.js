...
export default function CourseMaerialsMoreMenu({
  user,
  token,
  setLoadingBtn,
  loadingBtn,
  ...
  */
  ...
  const handleSubmitEditUnit = () => {
    ...
    axios
      .post(
        `${CourseVideoUnitsURL}${courseId}/materialUnit/${user.id}?_method=PUT`,
        data,
        {
          headers,
        }
  ...
  */
  ...
  const handleSubmitDeleteUnit = () => {
    ...
    axios
      .delete(`${CourseVideoUnitsURL}${courseId}/materialUnit/${user.id}`, {
        headers,
      })
      .then((response) => {
        setOpenDeleteUnit(false);
