...
export default function OldCourseVideosMoreMenu({
  user,
  token,
  setLoadingBtn,
  loadingBtn,
  ...
  */
  ...
  const handleSubmitEditUnit = () => {
    ...
    axios
      .post(
        `${CourseVideoUnitsURL}${courseId}/previous/videoUnits/${user.id}?_method=PUT`,
        data,
        {
          headers,
        }
  ...
  */
  ...
  const handleSubmitDeleteUnit = () => {
    ...
    axios
      .delete(
        `${CourseVideoUnitsURL}${courseId}/previous/videoUnits/${user.id}`,
        {
          headers,
        }
      )
